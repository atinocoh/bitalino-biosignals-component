...
void Filtro::insertarEnVentana(float valor){
    if (this->esVentanaLlena()){
        ...
        sumatorio=sumatorio+std::abs(valor);
        if (ventana.length()==tamVentanaMedia)
            ventanaLlena=true;
    }
}
...
void Filtro::insertarEnVentanaEnvolvente(QPointF valor){
    if(tamVentanaEnvolvente!=0){
        if (this->esVentanaEnvolventeLlena()){
            ...
            if (ventanaEnvolvente.length()==tamVentanaEnvolvente)
...
void Filtro::mediaMovil(QVector<qreal> &src, QVector<qreal> &dest){
    for (int var = 0; var < src.length(); ++var) {
        if (ventanaLlena){
            dest.push_back(sumatorio / tamVentanaMedia);
            this->insertarEnVentana(src.at(var));
        }
        else{
            dest.push_back(0);
            this->insertarEnVentana(src.at(var));
...
void Filtro::mediaMovil(const float src[], float dest[], int size){
    for (int var = 0; var < size; ++var) {
        if (ventanaLlena){
            dest[var] = (sumatorio / tamVentanaMedia);
            this->insertarEnVentana(src[var]);
        }
        else{
            dest[var] = 0;
            this->insertarEnVentana(src[var]);
 ...
 void Filtro::mediaMovilDownSample(const float src[], int size, QVector<QPointF> &dest, int downsample, int pagina){
     for (int var = 0; var < size; ++var) {
