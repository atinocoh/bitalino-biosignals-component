...
public:
    ...
    bool esVentanaLlena();
    void insertarEnVentana(float valor, bool valorAbsoluto);
    ...
    void mediaMovil(QVector<qreal> &src, QVector<qreal> &dest, bool valorAbsoluto);

    void mediaMovil(const float src[], float dest[], int size, bool valorAbsoluto);
    void mediaMovilDownSample(const float src[], int size, QVector<QPointF> &dest, int downsample,int pagina, bool valorAbsoluto);
};
